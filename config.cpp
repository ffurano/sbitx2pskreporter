...
void Config::SetString(const char *name, const char *val) {
  data[name] = val;
}
...
void Config::ArrayReset(const char *name) {
  if (arrdata.find(name) != arrdata.end()) {
    arrdata[name].clear();
  }
}

void Config::ArrayAddString(const char *name, const char *val) {
  arrdata[name].push_back(val);
}
